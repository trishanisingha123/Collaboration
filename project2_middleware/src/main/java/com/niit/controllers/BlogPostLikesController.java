...
return new ResponseEntity<BlogPostLikes>(blogPostLikes,HttpStatus.OK);	
	...
	@RequestMapping(value="/userliked/{blogPostId}",method=RequestMethod.GET)
	public ResponseEntity<?>userLikes(@PathVariable int blogPostId,HttpSession session)
	{
		String email=(String)session.getAttribute("email");
		...
		List<BlogPostLikes> blogPostLikes=blogPostLikesDao.userLikes(blogPostId);
		return new ResponseEntity<List<BlogPostLikes>>(blogPostLikes,HttpStatus.OK);
	}
}
